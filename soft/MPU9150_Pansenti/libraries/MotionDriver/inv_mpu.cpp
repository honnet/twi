...
int mpu_init(struct int_param_s *int_param)
{
  ...
  DP_int_ln(4);
    ...
    if (rev) {
...
#ifdef MPU_DEBUG
            DP_str("Unsupported software product rev: ");
            DP_int_ln(rev);
#endif
            return -4;
        }
        ...
        if (!rev) {
#ifdef MPU_DEBUG
            DP_str("Product ID read as 0 indicates device is either incompatible or an MPU3050");
#endif
            return -6;
        } else if (rev == 4) {
#ifdef MPU_DEBUG
            DP_str("Half sensitivity part found.\n");
#endif
            st->chip_cfg.accel_half = 1;
        } else
            st->chip_cfg.accel_half = 0;
...
#ifdef MPU_DEBUG
        DP_str("Unsupported software product rev: "); DP_int_ln(rev);
#endif
        return -7;
    }
...
#ifdef AK89xx_SECONDARY
#ifdef MPU_DEBUG
    DP_str("Setting up compass");
#endif
    errCode = setup_compass();
    if (errCode != 0) {
#ifdef MPU_DEBUG
       DP_str("Setup compass failed: "); DP_int_ln(errCode);
#endif
    }
...
#ifdef MPU_DEBUG
            DP_str("Firmware compare failed addr "); DP_int_ln(ii);
            for (int i = 0; i < 10; i++) {
              DP_int(progBuffer[i]);
              DP_str(" ");
            }
...
#ifdef MPU_DEBUG
    DP_str("Compass sens: "); DP_int(st->chip_cfg.mag_sens_adj[0]); DP_str(" ");
    DP_int(st->chip_cfg.mag_sens_adj[1]); DP_str(" ");
    DP_int(st->chip_cfg.mag_sens_adj[2]); DP_str(" ");
#endif
