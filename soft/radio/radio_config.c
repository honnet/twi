 ...
 *                         { 0x54, 0x2E, 0x57, 0x2E, 0x49 }  <===  "T.W.I"
 * hal_nrf_set_rf_channel(7);
 * hal_nrf_set_address_width(HAL_NRF_AW_5BYTES);
 * hal_nrf_set_address(HAL_NRF_TX, tx_address);     // TODO ?
 * hal_nrf_set_address(HAL_NRF_PIPE0, tx_address);  // TODO ?
 * hal_nrf_open_pipe(0, false);                     // TODO ?
 * hal_nrf_set_datarate(HAL_NRF_2MBPS);
 * hal_nrf_set_crc_mode(HAL_NRF_CRC_8BIT);
 * hal_nrf_setup_dynamic_payload(0xFF);             // TODO ?
 * hal_nrf_enable_dynamic_payload(false);           // TODO ?
 * @endverbatim
 *
...
void radio_configure()
{
    ...
    NRF_RADIO->FREQUENCY = 1UL;           // Frequency bin 1, 2401MHz
    NRF_RADIO->MODE      = (RADIO_MODE_MODE_Nrf_2Mbit << RADIO_MODE_MODE_Pos);

    // Radio address config
    NRF_RADIO->PREFIX1 =
        ((uint32_t)swap_bits(0x66) << 24) // Prefix byte of address 7 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 16) // Prefix byte of address 6 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 8)  // Prefix byte of address 5 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 0); // Prefix byte of address 4 converted to nRF24L series format
    NRF_RADIO->PREFIX0 =
        ((uint32_t)swap_bits(0x66) << 24) // Prefix byte of address 3 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 16) // Prefix byte of address 2 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 8)  // Prefix byte of address 1 converted to nRF24L series format
      | ((uint32_t)swap_bits(0x66) << 0); // Prefix byte of address 0 converted to nRF24L series format // 0x54 = 'T' ...

    // ... ".W.I"
    NRF_RADIO->BASE0       = bytewise_bitswap(0x66666666UL);  // Base address for prefix 0 converted to nRF24L series format
    NRF_RADIO->BASE1       = bytewise_bitswap(0x66666666UL);  // Base address for prefix 1-7 converted to nRF24L series format
    ...
    // Packet configuration
    NRF_RADIO->PCNF1 = (RADIO_PCNF1_WHITEEN_Disabled << RADIO_PCNF1_WHITEEN_Pos) | // TODO ?
